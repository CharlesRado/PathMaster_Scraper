...
def scrape_scopus():
    ...
    # Simplified query focusing just on robotics to get more results
    query = quote("robotics")
    
    # Updated URL with correct parameters
    ...
    try:
        ...
        if not results:
            print("No results found in Scopus response")
            return all_articles
        ...
        for item in results:
            try:
                ...
                # Skip if no URL (critical for deduplication)
                if not article_url:
                    continue
                ...
                # If no specific category but robotics-related, use general category
                if not categories:
                    # Ensure it's robotics related
                    combined_text = (title + " " + abstract).lower()
                    if "robot" in combined_text:
                        categories = ["General LLM & Robotics"]
                    else:
                        continue  # Skip if not robotics related
                ...
                for category in categories:
                    all_articles.append({
                        ...
                        "category": category,
                        "website": "Scopus",  # This is the field you're using as source
                        "timestamp": datetime.now().isoformat()
                    })
            except Exception as e:
